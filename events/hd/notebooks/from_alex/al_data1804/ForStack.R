...
y<-train$relevance
train <- read_csv('/home/ec2-user/moredata/al_data1804/pawel_train.csv')
test <- read_csv('/home/ec2-user/moredata/al_data1804/pawel_test.csv')
...
for (K in indxK) {
  ...
  x.mod.t  <- xgb.train(params = param, data = xgtrain , nrounds = NROUNDS)
  pr[currentIdx]<- predict(x.mod.t,xgtest)
  prt<-prt+ predict(x.mod.t,xgtestF)
}

tag_name <-paste('Apr21_', FEATSNAME, sep="")

results = data.frame(tag_name = pr)
results1 = data.frame(tag_name = prt/num_fold)
