...
object Script {
  ...
  def run(sc: SparkContext, sqlContext: SQLContext): Unit = {
    ...
    val (trainData, cvData, testData) = extract.data(hdfs = true, cv = 0.0002)  /// s3=true
    ...
    //    val (trainData, cvData, testData) = extract.data(s3 = true)  /// s3=true
    //    trainData.setName("trainData").cache()
        ...
        t0.polyline.takeRight(2) match {
          case lastSegmentBegin +: lastSegmentEnd +: Nil if t1.weekday == t0.weekday && t1.timeOfDay == t0.timeOfDay && t1.polyline.length < 240 =>
            ...
            if (found) {
              ...
              StatCounter(math.log(  15.0 * (t0.polyline.length + remainingInT1 - 1)   + 1))
            } else {
              StatCounter()
            }
    ...
    val results = cvData.map { case (actual, tripData ) =>
